...
The utility LLM command used for `summarize_chat` step context, the final task summary, AI task titles and slugs, and `sortie backfill-context`. The prompt is piped on **stdin**; the response must be printed on **stdout**. `SORTIE_PURPOSE` identifies the call site (`summarize`, `summarize_chat`, `summarize_chat_chunk`, `title`, `slug`, `backfill_context`).
