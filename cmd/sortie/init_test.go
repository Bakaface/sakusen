...
// scaffoldAgentFiles is the exact set of files `sortie init` copies into
// .sortie/agents/ (mirrors cmd/sortie/scaffold/agents/).
...
// TestInitCmd_HappyPath verifies that `sortie init` in a fresh git repo writes
// .sortie.yml, scaffolds exactly the embedded agent files with the right
...
func TestInitCmd_HappyPath(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(dir, ".sortie.yml")); err != nil {
		t.Errorf(".sortie.yml not created: %v", err)
	}

	agentsDir := filepath.Join(dir, ".sortie", "agents")
	...
	for _, want := range []string{".sortie/", runner.OutputLogFileName} {
		if !lines[want] {
...
// TestInitCmd_ScaffoldedConfigLoads verifies that the scaffolded .sortie.yml
...
func TestInitCmd_ScaffoldedConfigLoads(t *testing.T) {
	...
	if claude.Command != `"$SORTIE_PROJECT_PATH/.sortie/agents/claude-headless.sh"` {
	...
	if tmuxAgent.Command != `"$SORTIE_PROJECT_PATH/.sortie/agents/claude-tmux.sh"` {
		t.Errorf("claude-tmux command = %q", tmuxAgent.Command)
	}
	if tmuxAgent.ResumeCommand != `claude --dangerously-skip-permissions --resume "$SORTIE_SESSION_ID"` {
		t.Errorf("claude-tmux resume_command = %q", tmuxAgent.ResumeCommand)
	}
	if tmuxAgent.ChatLogCommand != `"$SORTIE_PROJECT_PATH/.sortie/agents/claude-chat-log.sh"` {
...
// .sortie.yml already exists: it must not scaffold the agents directory.
func TestInitCmd_ExistingConfig(t *testing.T) {
	dir := setupInitRepo(t)

	if err := os.WriteFile(filepath.Join(dir, ".sortie.yml"), []byte("max_workers: 1\n"), 0644); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := os.Stat(filepath.Join(dir, ".sortie")); !os.IsNotExist(err) {
		t.Errorf(".sortie dir should not be created on early return, stat err = %v", err)
	}
}
...
// .sortie.yml through config.ValidateFile — the strict (KnownFields) path
// `sortie validate` uses. LoadForProject silently drops unknown keys, so this
...
func TestInitCmd_ScaffoldedConfigPassesStrictValidation(t *testing.T) {
	...
	if err := config.ValidateFile(filepath.Join(dir, ".sortie.yml")); err != nil {
		t.Errorf("scaffolded .sortie.yml failed strict validation: %v", err)
	}
}
