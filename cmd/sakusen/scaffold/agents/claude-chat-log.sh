#!/usr/bin/env bash
# Scaffolded by `sakusen init` — user-owned; edit freely.
#
# Prints the step's chat transcript on stdout for sakusen's summarize_chat
# strategy. Sakusen runs this in the task workdir with:
#   SAKUSEN_TRANSCRIPT_PATH  transcript path from the latest turn-end sentinel
#                           (Claude's Stop-hook payload), when available
#   SAKUSEN_SESSION_ID       recorded agent session id, when available
# Falls back to locating the session JSONL under ~/.claude/projects/.
# Requires jq. Printing nothing means "no chat content" (sakusen skips the
# summary with a warning, or fails the step when require_context is set).
set -uo pipefail

transcript="${SAKUSEN_TRANSCRIPT_PATH:-}"
if [ -z "$transcript" ] && [ -n "${SAKUSEN_SESSION_ID:-}" ]; then
  # Claude Code encodes the workdir path by replacing every non-alphanumeric
  # character with '-'.
  encoded=$(printf '%s' "$PWD" | sed 's/[^a-zA-Z0-9]/-/g')
  candidate="$HOME/.claude/projects/$encoded/$SAKUSEN_SESSION_ID.jsonl"
