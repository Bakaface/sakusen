...
# turn-end sentinel into $SAKUSEN_DONE_DIR after every completed turn, which
# sakusen uses to auto-advance the workflow and to record the session id
...
if [ -s "${SAKUSEN_PROMPT_FILE:-}" ]; then
  exec claude "${ARGS[@]}" "$(cat "$SAKUSEN_PROMPT_FILE")"
else
  exec claude "${ARGS[@]}"
fi
