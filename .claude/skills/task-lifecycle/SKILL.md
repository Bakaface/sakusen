...
name: task-lifecycle
description: >
  Sakusen's task model: status state machine, priority system, dependency blocking,
...
**Title/slug refinement**: During `init`, an async goroutine generates an AI title via the configured `summarizer:` runner (`SAKUSEN_PURPOSE=title`, 30s timeout; with no summarizer, a sanitized truncated input is used) and, as a separate parallel call under the same timeout, an AI slug (`SAKUSEN_PURPOSE=slug`, resolved via `Config.SummarizerSlugInvocation()` — the `slug_agent:`/`slug_command:` override when set, else the shared `agent:`/`command:`). A caller-supplied title or slug skips the corresponding call; an empty input skips both. A failed slug call — including an answer that isn't a single usable token — falls back to `Slugify(title)` and never stalls the task. `FinalizeTaskIdentity()` updates title/slug/branch before transitioning to `pending`; a failed title call keeps the sanitized input as title but still finalizes the generated slug.
...
- `Slugify()`: lowercase, non-alphanumeric -> hyphens, trim, max 24 chars (`MaxSlugLength`) with word-boundary truncation (trims back to the last dash when the cut lands mid-word and that dash is past half the cap). It guards user-supplied and title-derived slugs. **AI-generated slugs never pass through it** — `generateSlug` (internal/daemon/handlers_task.go) uses the summarizer's answer verbatim (whitespace-trimmed, validated as a single letters/digits/`._-` token), so `MaxSlugLength` does not bound them.
