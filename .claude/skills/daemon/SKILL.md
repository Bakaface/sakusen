...
- `handleCreateTask`: creates task + async AI title/slug refinement via the configured `summarizer:` block (`workflow.RunSummarizer` — the agent file contract or a bare command; `SAKUSEN_PURPOSE=title` and `SAKUSEN_PURPOSE=slug` run in parallel under one 30s timeout; the slug call resolves via `cfg.SummarizerSlugInvocation()` and its answer becomes the slug verbatim — no `Slugify`, no length cap); a caller-supplied title/slug skips the matching call, and with no summarizer configured the title falls back to a sanitized truncated input and the slug to `task.Slugify(title)` (no error). For non-worktree tasks, branch resolution is skipped.
