...
// (SORTIE_PURPOSE=slug), and the answer becomes the task's slug and feeds the
...
func TestSummarizerGeneratesSlug(t *testing.T) {
	...
	e.MustSortie("create", "--title", "Manual Title", "add a login form with client-side validation")
	...
	if got := e.TaskField(1, "branch"); got != "sortie/1-ai-generated-slug" {
...
func TestExplicitSlugSkipsSummarizer(t *testing.T) {
	...
	e.MustSortie("create", "--title", "Manual Title", "--slug", "My Custom Slug", "add a login form")
