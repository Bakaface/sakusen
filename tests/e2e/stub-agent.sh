...
#
# Wired into test .sakusen.yml files as a headless agent command AND as the
# summarizer command. Sakusen's env contract drives everything — no argv:
#
...
#   $SAKUSEN_RESULT_FILE  where the final result text must be written (step spawns).
#
# Response files are looked up under $E2E_RESPONSES_DIR:
#   step spawns       → step-<step>.txt, else default.txt   (content becomes the
#                       step's result text, written to $SAKUSEN_RESULT_FILE;
...
# is emitted — used to simulate agents that touch files or call sakusen CLI.
#
# Side effects every call:
#   Appends one tab-separated line to $SAKUSEN_E2E_LOG:
#   <RFC3339Nano>\t<purpose>\t<cwd>\t<step>\t<env-pairs|…>
#   (env-pairs covers SAKUSEN_* and E2E_* vars, so tests can assert per-agent
...
# --- log invocation (TSV: timestamp, purpose, cwd, step, sakusen-env-pairs) ---
if [[ -n "${SAKUSEN_E2E_LOG:-}" ]]; then
    ENV_PAIRS=""
    for var in $(env | grep -E '^(SAKUSEN|E2E)_' | cut -d= -f1); do
        val="${!var:-}"
...
# --- opt-in prompt capture (step prompts arrive via $SAKUSEN_PROMPT_FILE) ---
if [[ -n "${E2E_PROMPT_CAPTURE_DIR:-}" && -n "${SAKUSEN_PROMPT_FILE:-}" && -f "${SAKUSEN_PROMPT_FILE:-}" ]]; then
    cp "$SAKUSEN_PROMPT_FILE" "$E2E_PROMPT_CAPTURE_DIR/prompt-${SAKUSEN_TASK_ID:-0}-${STEP_NAME:-none}.txt"
fi
...
case "$PURPOSE" in
    step|merge_conflict)
        ...
        # Result text goes into $SAKUSEN_RESULT_FILE. When the result file is
        ...
        # ONLY when $SAKUSEN_RESULT_FILE is unset.
        if [[ -n "${SAKUSEN_RESULT_FILE:-}" ]]; then
            emit_file "$STEP_FILE" > "$SAKUSEN_RESULT_FILE"
