...
// (newlines squashed) to logPath for every call and answers per SORTIE_PURPOSE.
...
func summarizerStub(logPath string, responses map[string]string) string {
	...
	return "p=$(cat); printf '%s\\t%s\\n' \"$SORTIE_PURPOSE\" \"$(printf '%s' \"$p\" | tr '\\n' ' ')\" >> " +
		shellQuote(logPath) + "; case \"$SORTIE_PURPOSE\" in " + cases.String() + "*) exit 1;; esac"
}
