...
// MaxSlugLength is the maximum length of a task slug. Slugs end up in branch
// names and worktree directory paths, so they are kept much shorter than
// titles.
const MaxSlugLength = 24

// Slugify converts arbitrary text into a kebab-case slug bounded by
// MaxSlugLength. It is the guardrail every slug passes through, whether it was
// generated by the summarizer, supplied by the user, or derived from a title.
func Slugify(title string) string {
	...
	if len(s) > MaxSlugLength {
		cut := s[:MaxSlugLength]
		// Only when the cut lands mid-word: trim back to the last dash, and
		// only when enough of the slug survives to stay meaningful. A cut that
		// already falls on a word boundary keeps the whole last word.
		if s[MaxSlugLength] != '-' {
			if idx := strings.LastIndexByte(cut, '-'); idx > MaxSlugLength/2 {
				cut = cut[:idx]
			}
		}
