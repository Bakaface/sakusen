...
// AI title and slug generation, and `sortie backfill-context`. The command is
// executed via `sh -c` with the prompt piped on STDIN and must print the
// response on stdout. SORTIE_PURPOSE identifies the call site ("summarize",
...
type SummarizerConfig struct {
	...
	// TitlePrompt overrides the built-in AI title prompt (SORTIE_PURPOSE=title).
	...
	// SlugPrompt overrides the built-in AI slug prompt (SORTIE_PURPOSE=slug),
