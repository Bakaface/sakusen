...
// TestAgentPromptField covers the agent `prompt:` field: parsing, variant
// inheritance/override, and alias propagation.
func TestAgentPromptField(t *testing.T) {
	isolateHome(t)
	yaml := `
agents:
  base:
...
`
	...
	base, ok := cfg.ResolveAgent("base")
	if !ok || !strings.Contains(base.Prompt, "Be lazy first.") {
		t.Fatalf("base.Prompt = %q, want the parsed agent prompt", base.Prompt)
	}
	if quiet, ok := cfg.ResolveAgent("base:quiet"); !ok || quiet.Prompt != base.Prompt {
		t.Errorf("base:quiet prompt = %q, want the inherited parent prompt", quiet.Prompt)
	}
	if loud, ok := cfg.ResolveAgent("base:loud"); !ok || loud.Prompt != "LOUD {{prompt}}" {
		t.Errorf("base:loud prompt = %q, want the variant's wholesale override", loud.Prompt)
	}
	if alias, ok := cfg.ResolveAgent("implementer"); !ok || alias.Prompt != "LOUD {{prompt}}" {
		t.Errorf("implementer prompt = %q, want the alias target's prompt", alias.Prompt)
	}
	if plain, ok := cfg.ResolveAgent("plain"); !ok || plain.Prompt != "" {
		t.Errorf("plain.Prompt = %q, want empty", plain.Prompt)
	}
}

// TestMergeConflictAgentResolution covers the merge_conflict_agent cascade:
// the explicit key wins, and without it the resolver falls back to the
// workflow agent (with the implicit "claude" fallback for tmux workflows).
func TestMergeConflictAgentResolution(t *testing.T) {
	isolateHome(t)
	yaml := `
agents:
  claude:
...
`
	...
	wf := cfg.GetTaskWorkflow("tmux-wf")
	slug, agent, err := cfg.MergeConflictAgentFor(wf)
	if err != nil {
		t.Fatalf("MergeConflictAgentFor: %v", err)
	}
	if slug != "claude" || agent.Command != "claude-cmd" {
		t.Errorf("MergeConflictAgentFor = (%q, %q), want the claude fallback for a tmux workflow agent", slug, agent.Command)
	}

	cfg.MergeConflictAgent = "fixer"
	slug, agent, err = cfg.MergeConflictAgentFor(wf)
	if err != nil {
		t.Fatalf("MergeConflictAgentFor with explicit key: %v", err)
	}
	if slug != "fixer" || agent.Command != "fixer-cmd" {
		t.Errorf("MergeConflictAgentFor = (%q, %q), want the explicit merge_conflict_agent", slug, agent.Command)
	}
}

// TestMergeConflictAgentTierMerge verifies merge_conflict_agent parses and
// merges across tiers exactly like default_agent (more-local wins).
func TestMergeConflictAgentTierMerge(t *testing.T) {
	_, homeDir, _ := writeGlobalConfigYaml(t, `
agents:
  global-fixer:
    command: global-cmd
merge_conflict_agent: global-fixer
`)
	...
	if cfg.MergeConflictAgent != "global-fixer" {
		t.Errorf("MergeConflictAgent = %q, want global-fixer from the global tier", cfg.MergeConflictAgent)
	}

	if err := os.WriteFile(filepath.Join(homeDir, ".sakusen.yml"), []byte(
		"agents:\n  home-fixer:\n    command: home-cmd\nmerge_conflict_agent: home-fixer\n"), 0644); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(projectDir, ".sakusen.yml"), []byte(
		"agents:\n  proj-fixer:\n    command: proj-cmd\nmerge_conflict_agent: proj-fixer\n"), 0644); err != nil {
		t.Fatal(err)
	}
	cfg, err = LoadForProject(projectDir)
	if err != nil {
	...
	if cfg.MergeConflictAgent != "proj-fixer" {
		t.Errorf("MergeConflictAgent = %q, want the project tier to win", cfg.MergeConflictAgent)
	}
}

// TestMergeConflictAgentLoadErrors covers the load-time checks on an explicit
// merge_conflict_agent: the slug must resolve and must be headless.
func TestMergeConflictAgentLoadErrors(t *testing.T) {
	tests := []struct {
		name    string
		yaml    string
		wantErr string
		...
		{
			name:    "unknown slug",
			yaml:    "agents:\n  a:\n    command: x\nmerge_conflict_agent: nope\n",
			wantErr: `merge_conflict_agent: unknown agent "nope"`,
		},
		{
			name:    "tmux-mode agent",
			yaml:    "agents:\n  a:\n    mode: tmux\n    command: x\nmerge_conflict_agent: a\n",
			wantErr: "conflict resolution cannot use a tmux-mode agent",
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := LoadForProject(dir)
			if err == nil || !strings.Contains(err.Error(), tt.wantErr) {
				t.Fatalf("LoadForProject error = %v, want it to contain %q", err, tt.wantErr)
			}
		})
	}
}
