...
- **Step context comes from the agent's result text, stored in DB after each step** — headless steps read it from `$SORTIE_RESULT_FILE` (stdout-tail fallback); tmux steps with `summarize_chat` get their chat via the agent record's `chat_log_command`. Synchronous capture; step index persisted
- **The tmux sentinel convention is generic** — the engine exports `SORTIE_DONE_DIR` (`StepDoneDir`) and `SORTIE_DONE_PREFIX` (`SentinelPrefix(step)`); whatever runs in the session (a hook, the agent, an idle-watcher) signals turn-end by writing `$SORTIE_DONE_DIR/$SORTIE_DONE_PREFIX-<ts>.json`, whose optional JSON payload may carry `session_id`/`transcript_path`. Core installs NO Stop hook — hookless tmux agents are manual-advance
