...
// Arbitrary-track edits are a human operation (the `sortie tracks
// set-description` CLI), not an agent one.
type UpdateTrackDescriptionArgs struct {
	TaskID      int64  `json:"task_id" jsonschema:"The calling task's ID (from $SORTIE_TASK_ID). Required. This tool only writes to the calling task's own track — the daemon rejects tasks with no track or no active step."`
...
func registerUpdateTrackDescription(s *server.MCPServer, c *client.Client) {
	tool := mcp.NewTool(
		"update_track_description",
		mcp.WithDescription("Set the description of the calling task's own track — the stable one-liner that states the track's purpose and primes routing agents choosing a track in list_tracks. Only writes to the calling task's track — pass your own task_id (from $SORTIE_TASK_ID); the daemon rejects tasks with no track or no active step. Replace-only: the new description overwrites the old one. Use update_track_context, not this tool, to accumulate working context."),
...
func handleUpdateTrackDescription(c *client.Client, args UpdateTrackDescriptionArgs) (*mcp.CallToolResult, error) {
	...
	// human operation (`sortie tracks set-description`).
