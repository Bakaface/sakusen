...
func registerCreateTrack(s *server.MCPServer, c *client.Client) {
	tool := mcp.NewTool(
		"create_track",
		mcp.WithDescription("Create a new track — a named, mutable context container that tasks attach to at create time. A track's (parent-concatenated) context is injectable into workflow step prompts via {{track.context}} / {{track.own_context}}, so agents can accumulate shared context across a stream of related tasks. Returns the created track as JSON."),
